<template>
  <div class="parent-container">
    <div class="child-container">
      ...
      <q-linear-progress class="q-mb-md" color="primary" size="20px" indeterminate v-if="!store.keysLoaded" />
      <div class="q-mb-md">
        <q-checkbox keep-color v-model="numKeys" color="teal"> Num </q-checkbox>
        <q-checkbox keep-color v-model="engKeys" color="orange"> Eng </q-checkbox>
        <q-checkbox keep-color v-model="rusKeys" color="red"> Rus </q-checkbox>
        <q-checkbox keep-color v-model="specKeys" color="cyan"> Spec </q-checkbox>
      </div>
      <table ref="tableRef">
        <tr>
          <td colspan="16">
        ...
        <tr>
          <td colspan="30">
            <div class="toggle-wrapper">
              <q-toggle color="primary" :label="toggleKeyboardLabel" keep-color v-model="showKey" />
            </div>
          </td>
        </tr>
        <template v-if="showKey">
          ...
          >
            <td
              ...
              :class="{
                yellow: symbolToRemove === value,
                pink: symbolToRemove !== value,
              }"
            >
        ...
        <tr v-for="(item, index) in chunkedKeyValuePairs(allSymbols, symbolsInRow)" :key="index" class="text-center">
          <td
            ...
            :class="{
              yellow: symbolToAdd === symbol,
              blue: symbolToAdd !== symbol,
            }"
          >
            {{ symbol }}
          </td>
      ...
      <q-btn @click="randomizeKey" class="btn q-mt-md q-mr-md" no-caps :disable="!store.currentKeyboardId"
...
const chooseSymbolsToChange = (symbol, keyboard, clickOnSymbols = false) => {
  ...
  if (
    clickOnSymbols &&
    Object.values(keyboard).find((el) => el === symbol) &&
